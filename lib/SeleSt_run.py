...
Selective Stopping (SeleSt) task
    ...
    SeleSt_run
        IFunctions to run the task can be found in this script
...
def Block(exp,trialInfo):
    ...
    if exp.genSettings['Import trials?'] == False: # don't shuffle trials if they have been imported
        shuffleTrials = True
        while shuffleTrials == True:
            thisBlockTrials = trialInfo.trialList
            shuffle(thisBlockTrials)
            if sum(thisBlockTrials[:exp.advSettings['n forced go trials']]) > exp.advSettings['n forced go trials']: # reshuffle trials if n forced go criterion is not met
                thisBlockTrials = shuffle(thisBlockTrials)
            elif sum(thisBlockTrials[:exp.advSettings['n forced go trials']]) <= exp.advSettings['n forced go trials']: # break loop if n forced go criterion is achieved
                break    
    elif exp.genSettings['Import trials?'] == True:
        thisBlockTrials = trialInfo.blockTrials[trialInfo.blockCount] # start from 0 to acocunt for zero-based array index
    # Insert block of practice trials if option is selected
...
class Initialize_trial:
    def __init__(self,exp,trialInfo,stopInfo,trial):
        if exp.genSettings['Import trials?'] == True: # if parameters are specified in imported trials file
...
class Start_Trial:
    def __init__(self,exp,stimuli,thisTrial):
        if exp.genSettings['Import trials?'] == True: # use imported trial information if selected
        ...
        if exp.genSettings['Paradigm'] == 'SST': # reset dynamic SST parameters
...
def fixationPeriod(exp):
    ...
    if exp.advSettings['Hold-and-release'] == True: # if hold and release
        L_key = 0
        R_key = 0
...
def getRT(exp,thisTrial):
    if thisTrial.L_press == 0: # if no press assign RTs of 0
        ...
        if exp.advSettings['Hold-and-release'] == True: # use key duration if hold-and-release
    ...
    if thisTrial.R_press == 0:
        ...
        if exp.advSettings['Hold-and-release'] == True:
...
def feedback(exp,stimuli,trialInfo,thisTrial): 
    if exp.genSettings['Paradigm'] == 'ARI': # use positional RTs if ARI paradigm
        L_RT = thisTrial.L_RT_pos
        R_RT = thisTrial.R_RT_pos
    elif exp.genSettings['Paradigm'] == 'SST': # use stored RTs if SST paradigm
    ...
    if exp.genSettings['Feedback?'] == True: # draw feedback if option is selected
        exp.win.flip()
...
def staircaseSSD(exp,stopInfo,thisTrial): 
    ...
    if exp.advSettings['Staircase stop-signal delays?'] == True: # only staircase if option is enabled
...
def saveData(exp,trialInfo,thisTrial,startTime):
    if exp.genSettings['Save data?'] == True: # save data if option is selected
...
def endBlock(exp,trialInfo,thisBlockTrials):
    ...
    if exp.genSettings['Feedback?'] == True:
